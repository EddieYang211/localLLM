...
## Feature: Windows Vulkan GPU Backend (Plan A+B)

**Rationale**: Vulkan covers NVIDIA + AMD + Intel Arc with a single build. Unlike CUDA, the Vulkan runtime (`vulkan-1.dll`) ships with every modern GPU driver — no separate toolkit installation is needed by the end user. The zip artifact stays small (~5–10 MB vs ~220 MB for CUDA), and detection is a single file-existence check. llama.cpp's Vulkan backend (`ggml-vulkan`) is production-ready and broadly used.

CUDA was validated locally (RTX 2080 Super, ~41x speedup) and remains an option for NVIDIA-only power users in the future, but Vulkan is the better default for a distributed R package.

---

### Part 1: CI/CD — Vulkan Build in `release-builder.yml`

#### 1.1 Remove the CUDA matrix entry, add a Vulkan entry

Replace the `Windows (CUDA 12.4)` matrix entry added in a previous iteration with:

```yaml
- os: windows-latest
  os_name: Windows (Vulkan)
  artifact_name: localllm_windows_x64_vulkan.zip
  lib_name: localllm.dll
  vulkan: true
```

Keep the existing `Windows (CPU)` entry unchanged as the fallback.

#### 1.2 Install Vulkan SDK (headers + loader, compile-time only)

Add a conditional step before configure. The `humbletim/setup-vulkan-sdk` action is the standard approach for Windows CI:
...
This installs headers and `vulkan-1.lib` (the import library needed at link time). It does NOT install `vulkan-1.dll` — that comes from the user's GPU driver at runtime.

#### 1.3 CMake configure change

In the configure step, add `-DGGML_VULKAN=ON` for Vulkan builds:

```bash
if [[ "${{ matrix.vulkan }}" == "true" ]]; then
  CMAKE_ARGS="$CMAKE_ARGS -DGGML_VULKAN=ON"
  echo "Vulkan GPU backend enabled"
fi
```

No generator change needed — the default Visual Studio generator works fine with Vulkan (unlike CUDA which required Ninja + MSVC dev env).

#### 1.4 Packaging — no extra DLLs needed

`vulkan-1.dll` is already present on any Windows machine with an installed GPU driver (NVIDIA, AMD, or Intel). Nothing needs to be bundled alongside `localllm.dll`.

**Expected zip contents**:

| File | Size | Notes |
|------|------|-------|
| `localllm.dll` | ~5–10 MB | Vulkan-enabled inference engine |

Compare to the CUDA approach (~220 MB with bundled cuBLAS). No PATH manipulation required at load time either.

#### 1.5 Remove CUDA steps from `release-builder.yml`

Remove the steps added in the previous CUDA iteration:
- `Install CUDA Toolkit 12.4` step
- `Setup MSVC Developer Environment` step
- The `cuda_version` / `cuda_arch` matrix fields
- The DLL bundling loop for CUDA DLLs

The `ilammy/msvc-dev-cmd` and CUDA download steps are no longer needed.

---

### Part 2: `install.R` — Automatic Vulkan Detection (Plan B)

#### 2.1 New helper: `.detect_vulkan_windows()`

Vulkan detection is a single file check — far simpler than CUDA detection:

```r
.detect_vulkan_windows <- function() {
  if (.Platform$OS.type != "windows") return(FALSE)
  # vulkan-1.dll is present on any Windows system with a modern GPU driver installed
  vulkan_dll <- file.path(Sys.getenv("SystemRoot"), "System32", "vulkan-1.dll")
  file.exists(vulkan_dll)
}
```

#### 2.2 Modify `.get_download_url()`
...
.get_download_url <- function(use_gpu = NULL) {
  ...
  if (sys == "Darwin") {
    ...
    if (arch == "x86-64") {
      gpu_available <- if (is.null(use_gpu)) .detect_vulkan_windows() else use_gpu
      if (gpu_available) {
        filename <- "localllm_windows_x64_vulkan.zip"
        .localllm_message("GPU detected (Vulkan) — downloading GPU-accelerated build")
      } else {
  ...
  if (is.null(filename)) {
    stop("Your platform (", sys, "/", arch, ") is not currently supported.", call. = FALSE)
  }
...
#### 2.3 Modify `install_localLLM()` — add `force_cpu` escape hatch

```r
install_localLLM <- function(force_cpu = FALSE) {
  if (lib_is_installed()) {
    .localllm_message("localLLM backend library is already installed.")
    return(invisible(NULL))
  }

  gpu_available <- !force_cpu && .detect_vulkan_windows()

  if (interactive()) {
    ans <- utils::askYesNo(
      paste0(
        "The localLLM C++ backend library is not installed.\n",
        "This will download pre-compiled binaries (~5 MB) to your local cache.\n",
        if (gpu_available) "A Vulkan-capable GPU was detected — the GPU-accelerated build will be installed.\n" else "",
        "Do you want to proceed?"
      ),
      default = TRUE
    )
    if (!ans) stop("Installation cancelled by user.", call. = FALSE)
  }

  download_url <- .get_download_url(use_gpu = gpu_available)
  # ... rest of install logic unchanged ...
}
```

No changes needed to the DLL loading code in `zzz.R` — `vulkan-1.dll` is found through the system PATH automatically.
...
|------|------|--------|-------------|
| 1 | `release-builder.yml` | ~20 lines | Replace CUDA matrix entry with Vulkan; add `humbletim/setup-vulkan-sdk` step; add `-DGGML_VULKAN=ON` to configure; remove all CUDA steps |
| 2 | `localLLM/R/install.R` | ~40 lines | Add `.detect_vulkan_windows()`; update `.get_download_url()`; add `force_cpu` param to `install_localLLM()` |
| 3 | Publish new Release | — | CI produces two Windows zips: `localllm_windows_x64.zip` (CPU) and `localllm_windows_x64_vulkan.zip` (Vulkan) |
| 4 | Test | — | On a machine with any modern GPU: verify Vulkan build is auto-selected and loads correctly |

---

### Notes

- **GPU coverage**: NVIDIA (GeForce 10xx+), AMD (RX 400+), Intel Arc — all Vulkan 1.1+ capable. Integrated Intel UHD graphics also supports Vulkan but performance will be modest.
- **Vulkan vs CUDA performance**: Vulkan is slightly slower than CUDA on NVIDIA hardware due to less-optimized kernels, but the gap has narrowed considerably in recent llama.cpp versions. For most R users the difference is not significant.
- **`force_cpu = TRUE`**: Lets users opt out of GPU inference, e.g. `install_localLLM(force_cpu = TRUE)`. Useful when the GPU has insufficient VRAM for the chosen model.
- **Upgrading from CPU build**: `lib_is_installed()` returns `TRUE` for existing installs, so the GPU build is not auto-applied on upgrade. Users can force a reinstall by deleting the cache directory returned by `get_lib_path()`.
- **Future CUDA option**: If NVIDIA-specific optimizations become important, a `localllm_windows_x64_cuda12.zip` can be added as an additional matrix entry without touching the Vulkan or CPU paths.
