...
For larger models the speedup is more pronounced because the KV-cache and attention computation per token are heavier, making batching more efficient relative to sequential overhead.
...
|---|---|---|---|
| Llama-3.2-3B-Instruct-Q5_K_M | ~48 sec | ~31 sec | ~1.5x |
| DeepSeek-R1-Distill-Qwen-14B-Q5_K_M | 617.2 sec | 242.0 sec | **2.55x** |

The 14B model achieves a larger relative speedup (2.55×) because larger models are more
memory-bandwidth-bound — batching amortises that cost across sequences more effectively.
