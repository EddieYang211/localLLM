...
When working with LLMs for research, you often need to compare multiple models systematically. The `explore()` function manages batch annotation across models, while `validate()` computes confusion matrices and reliability metrics.
...
For very large datasets, stream results to disk to keep memory flat:

```{r}
# Stream annotations to CSV as they're generated
annotations <- explore(
  models = models,
  prompts = template_builder,
  sink = annotation_sink_csv("annotations.csv"),
  batch_size = 50,
  engine = "parallel"
)

# Results are written incrementally to annotations.csv
```
